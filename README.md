# LOPOOP - 로스트아크 길드 관리 애플리케이션

LOPOOP은 한국 로스트아크 플레이어를 위한 길드 관리 애플리케이션입니다. 길드원들이 레이드 스케줄링, 빚 관리, 모험섬 정보를 효율적으로 관리할 수 있도록 도와줍니다.

## 주요 기능

### 🎯 레이드 스케줄링
- 4슬롯 팀 구성으로 레이드 파티 추적
- 캐릭터 이름, 직업, 역할(서폿/딜러) 표시
- 아이템 레벨 기반 파티 구성
...
### 👤 개인 페이지
- **개인 빚 관리**: 공통 빚과 별도로 개인 빚을 관리
- **개인 스케줄**: 개인 일정과 레이드 스케줄을 따로 관리
- **개인 설정**: 개인화된 설정 관리
...
2. Get your Supabase anon key from the [Supabase Dashboard](https://supabase.com/dashboard/project/lqouhidmuczumzrkpphc/settings/api)
